...
let customScale = 1;
let targetFps = 60;
let bgmfile = './assets/audio/Daily Routine 247.flac';
let bgmvolume = 0;
let bgm;
...
const AUDIO_DETAIL = [
	{ 
		time: 12000, 
		count: 2, 
		startTimes: [ 1000, 6300 ], 
		dialog: { 
			Japanese: [
				"…先生は、騙されたんだよ。",
				"どうしようね？もう、朝までここにいるしかないよ？"
			],
			English: [
	...
	{ 
		...
		dialog: { 
			Japanese: [
				"こーんな簡単に私を信じちゃうなんて、先生ってホント…",
				"あれほど「私を信じないで」って言ったのに…"
			],
			English: [
	...
	{ 
		time: 17000, 
		count: 2, 
		startTimes: [ 500, 10500 ], 
		dialog: { 
			Japanese: [
				"ん？ここで騒いだら…みんなにバレちゃうよ？",
				"いいの？…私は、別にいいけど。"
			],
			English: [
				"Shhh! If you cause a scene here, everyone will find out.",
				"Of course, it doesn't matter to me either way. I've got nothing to lose."
			],
			Chinese: [
				"嗯？在这里发出什么动静的话…会被大家发现的哦？",
				"这样也无所谓吗？…我倒不是很在乎呢。"
			]
		} 
	},
	{ 
		time: 17500, 
		count: 3, 
		startTimes: [ 500, 4800, 10000 ], 
		dialog: { 
			Japanese: [
				"…あれ。",
				"もしかして…怒った？",
				"私が問題児だってことくらい、先生だって知ってたでしょ…？"
			],
			English: [
	...
	{ 
		time: 33000, 
		count: 4, 
		startTimes: [ 500, 7100, 15400, 24650 ], 
		dialog: { 
			Japanese: [
				"その…困らせたかったんじゃないの。",
				"先生に、嫌われるのは…やだ…",
				"でも…ちょっとだけ…ほんのちょっとでいいから…",
				"先生と、一緒にいたかったの…。"
			],
			English: [
...
const HITBOX = {
	headpat: { xMin: 1950, xMax: 2500, yMin: 100, yMax: 920 },
	voiceline: { xMin: 970, xMax: 1630, yMin: 580, yMax: 1600 }
};
